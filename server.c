...
int main(){
	...
	const struct sockaddr_in addr = {
		...
		.sin_port = 0x901f, /* Refers to 8080. Port 8080 is 0x1f90 in hex,
	...
	int l = listen(s, 10); // 10 connections at a time is enough for POC
	if (l){
	...
	/* Return -1 when error(s) occur, otherwise return the number of byte received */
	/* Only use this function with connected socket */
	char data_buffer[256] = {0}; // Buffer to store received data
	...
	if (rec < 0){
		...
		printf("Data received: %ld byte(s).\n", strlen(data_buffer));
	}
	...
	if (*strchr(fn, ' ')){ // strchr() return a pointer to needed character. If not found, return NULL.
		printf("Character not found. Maybe HTTP request was broken. Exiting ...\n");
		return 0;
	}
	else{
