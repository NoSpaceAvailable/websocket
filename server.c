...
int main(){
	...
	/* It returns a socket descriptor or -1 on failure, so I save it in variable 's' to manage */
	...
	/* Initialize a sockaddr_in object to store data stream */
	...
	/* Once the socket opened successfully, bind it with a port */
	...
	/* Once the port binded successfully, listen for incoming connections */
	...
	/* accept() function has three params:
	   ...
	   -	struct sockaddr *addr: a pointer to the socket which belongs to the incoming connection (a.k.a client).
				       If 0 is assigned, it means that the server will accept the connection but doesn't know
				       any information about it, like the IP address. If you want to collect information,
				       you have to init a sockaddr object for the client and pass the pointer as a parameter.
				       After that, you can extract info from sockaddr's properties.
	   -	socklen_t *addrlen: this is a value-result argument, which is a declared term: "Value-result argument is an
				    argument that its value can be changed when passed to a function". You can initialize
				    it like this: socklen_t sockSize = sizeof(client_socket). This argument is used to tell the
				    accept() function the actual size of the client socket address. addrlen's value and the actual size
				    of the client socket address can be different, potentially resulting in data loss when transferring data.
				    So, it's necessary to pass a pointer *addrlen instead of only passing its value. The accept() function
		...
		If sockaddr is NULL, the server only accepts connections from clients but doesn't need to know anything about it.
	...
	char data_buffer[256]; // Buffer to store received request.
	/* When a request is accepted, receive data from it */
	/* Return -1 when error(s) occur. Otherwise, return the number of bytes received. When end-of-file is reached, return 0 */
	/* Only use this function with a connected socket */
	...
	/* Transfer data from the client's file descriptor to the server's file descriptor */
	while (true){
		int sent = sendfile(client_fd, server_fd, NULL, 256); /* client_fd is the file to write server_fd is the file to read,
								 	 NULL pass to off_t *offset (a pointer to the position that this
								 	 function should read in server_fd) means just read from the start of the file.*/
		if (sent < 0){
